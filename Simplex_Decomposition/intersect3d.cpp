...
{
    N[0]=0;
    Coord_3D A(V0[0],V0[1],V0[2]);
    Coord_3D B(V1[0],V1[1],V1[2]);
    Coord_3D C(V2[0],V2[1],V2[2]);
    Coord_3D D(U0[0],U0[1],U0[2]);
    Coord_3D E(U1[0],U1[1],U1[2]);
    Coord_3D F(U2[0],U2[1],U2[2]);
...
int intersect(Coord_3D *A1, Coord_3D *A2, Coord_3D *A3, Coord_3D *B1, Coord_3D *B2, Coord_3D *B3){
    ...
    setPoint(p0, A1->x, A1->y, A1->z);
    setPoint(p1, A2->x, A2->y, A2->z);
    setPoint(p2, A3->x, A3->y, A3->z);
    setPoint(q0, B1->x, B1->y, B1->z);
    setPoint(q1, B2->x, B2->y, B2->z);
    setPoint(q2, B3->x, B3->y, B3->z);

    computeEdges(v0, v1, p0, p1, p2);
    computeEdges(w0, w1, q0, q1, q2);
    return tr_tri_intersect3D(p0, v0, v1, q0, w0, w1);

}

int intersectF(Face *F1, Face *F2, vector<Coord_3D *> *Pontos){
    return intersect(Pontos->at(F1->P1),Pontos->at(F1->P2),Pontos->at(F1->P3),Pontos->at(F2->P1),Pontos->at(F2->P2),Pontos->at(F2->P3));
}
